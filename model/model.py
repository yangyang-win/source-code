...
class BiFPNDecoder(nn.Module):
    def __init__(
        ...
        self.seg_p5 = SegmentationBlock(384, 64, n_upsamples=4)
        self.convd = nn.Conv2d(128, 64, kernel_size=3, padding=1)
        #bn2 = nn.BatchNorm2d(in_channels)
        
        self.merge1 = MergeBlock(merge_policy)
        self.merge2 = MergeBlock(merge_policy)
    ...
    def forward(self, inputs):
        ...
        x = self.convd(x)
        #x = self.merge((p2,cp3,cp4,cp5,p3,p4,p5))
...
class SegmentationHead(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, activation=None, upsampling=1):
        conv2d1 = nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1)
        bn2 = nn.BatchNorm2d(in_channels)
        conv2d2 = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=kernel_size // 2)
        #bn2 = nn.BatchNorm2d(in_channels)
    #    conv2d2 = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1)
        upsampling = nn.UpsamplingBilinear2d(scale_factor=upsampling) if upsampling > 1 else nn.Identity()
        activation = Activation(activation)
        super().__init__(conv2d1, bn2, conv2d2, upsampling, activation)
